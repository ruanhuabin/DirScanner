...
def mergeStatFiles(outputDir, fileNamePrefix,  processNum, statFileName):
    ...
    allStatInfo = []
    for i in xrange(0, processNum):
        fileName = outputDir + fileNamePrefix + "_{0}".format(i) + "_stat.txt"
        with open(fileName) as f:
            for line in f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--process_num", action="store", type=int, help="The process number to run", default=1)
    ...
    mergeStatFiles(intermediateResultDir, fileNamePrefix, processNum, outputStatFileName)
    time.sleep(3)
    ...
    exit()
    if(op == "gen_file_path_only"):
        if (bool(args.dir_to_search) == False):
            parser.error('--operation gen_file_path_only and --dir_to_search should appear simultaneously')
            exit()
        else:
            dirToSearch = args.dir_to_search;
            operation = "gen_file_path_only"
            currTime = getCurrDateTime()
            allFilePathResultFile = args.output_dir + "/all_path_" + currTime + ".txt"
            ignoredDirs = args.exclude_dirs
            logger.info("Search in directory: " + dirToSearch)
            logger.info("Dirs to be ignored: " + str(ignoredDirs))
            logger.info("Operation is: " + operation)
            logger.info("Output file name: " + allFilePathResultFile)
            logger.info("Start to gen file path list")
            genFilePathList(dirToSearch, allFilePathResultFile, ignoredDirs, logger)
            logger.info("End to gen file path list")
            exit()
    elif(op == "gen_stat_file"):
        if (bool(args.dir_to_search) == False):
            parser.error('--operation gen_stat_file and --dir_to_search should appear simultaneously')
            exit()
        else:
            dirToSearch = args.dir_to_search;
            operation = "gen_stat_file"
            currTime = getCurrDateTime()
            allFilePathResultFile = args.output_dir + "/all_path_" + currTime + ".txt"
            statResultFile = args.output_dir + "all_stat_" + currTime + ".txt"
            statInvalidResultFile = args.output_dir + "all_stat_invalid_" + currTime + ".txt"
            ignoredDirs = args.exclude_dirs
            logger.info("Search in directory: " + dirToSearch)
            logger.info("Dirs to be ignored: " + str(ignoredDirs))
            logger.info("Operation is: " + operation)
            logger.info("Output file path file name: " + allFilePathResultFile)
            logger.info("Output stat result file name: " + statResultFile)
            logger.info("Output stat result invalid info file name: " + statInvalidResultFile)
            logger.info("Start to gen file path list")
            genFilePathList(dirToSearch, allFilePathResultFile, ignoredDirs, logger)
            logger.info("End to gen file path list")
            logger.info("Start to gen stat info")
            getStatInfo(allFilePathResultFile, statResultFile, statInvalidResultFile, logger)
            logger.info("End to gen stat info")
            exit()
    elif(op == "gen_type_typeperiod_2_num_size"):
        if (bool(args.stat_file_name) == False):
            parser.error('--operation gen_type_typeperiod_2_num_size and --stat_file_name should appear simultaneously')
            exit()
        else:
            statFileName= args.stat_file_name;
            operation = "gen_type_typeperiod_2_num_size"
            currTime = getCurrDateTime()
            type2NumSizeFile= args.output_dir + "/all_type_2_num_size_" + currTime + ".txt"
            typeTypePeriod2NumSizeFile = args.output_dir + "all_type_typeperiod_2_num_size_" + currTime + ".txt"
            baseTime = args.base_time
            logger.info("Input stat file name: " + statFileName)
            logger.info("Operation is: " + operation)
            logger.info("Output type to num_size file: " + type2NumSizeFile)
            logger.info("Output type typeperiod to num size: " + typeTypePeriod2NumSizeFile)
            logger.info("Base time is: " + baseTime)
            logger.info("Start to type and typeperiod to num_size file")
            genTypeTypePeriod2NumSize(statFileName, typeTypePeriod2NumSizeFile, type2NumSizeFile, baseTime) 
            logger.info("End to type and typeperiod to num_size file")
            exit()
    elif(op == "gen_user_grp_2_num_size"):
        if (bool(args.stat_file_name) == False):
            parser.error('--operation gen_user_grp_2_num_siz and --stat_file_name should appear simultaneously')
            exit()
        else:
            outputDir = args.output_dir
            if(outputDir[-1] != '/'):
            ...
            operation = "gen_type_typeperiod_2_num_size"
            currTime = getCurrDateTime()
            user2UsageFile = outputDir+ "all_user_2_usage_" + currTime + ".txt"
            grp2UsageFile= outputDir + "all_grp_2_usage_" + currTime + ".txt"
            grp2UserFile= outputDir + "all_grp_2_user_" + currTime + ".txt"
            logger.info("Input stat file name: " + statFileName)
            logger.info("Operation is: " + operation)
            logger.info("Output user to num_size file: " + user2UsageFile)
            logger.info("Output group to to num size file: " + grp2UsageFile)
            logger.info("Output group to to user file: " + grp2UserFile)
            logger.info("Start to type and typeperiod to num_size file")
        ...
        if (bool(args.dir_to_search) == False):
            parser.error('Please specify a directory to scan.')
            exit()
        else:
            outputDir = args.output_dir
            if(outputDir[-1] != '/'):
            ...
            operation = "all"
            currTime = getCurrDateTime()
            pathListFileName = outputDir + "all_file_path_" + currTime + ".txt"
            statListFileName = outputDir + "all_stat_" + currTime + ".txt"
            statInvalidListFileName = outputDir + "all_stat_invalid_" + currTime + ".txt"
            type2NumSizeFile = outputDir+ "all_type_2_num_size_" + currTime + ".txt"
            typeTypePeriod2NumSizeFile = outputDir + "all_type_typeperiod_2_num_size_" + currTime + ".txt"
            user2UsageFile = outputDir+ "all_user_2_usage_" + currTime + ".txt"
            grp2UsageFile= outputDir + "all_grp_2_usage_" + currTime + ".txt"
            grp2UserFile= outputDir + "all_grp_2_user_" + currTime + ".txt"
            logger.info("Dir to search: " + dirToSearch)
            logger.info("Operation is: " + operation)
            logger.info("Excluede dirs: " + str(ignoredDirs))
            logger.info("Base time is: " + baseTime)
            logger.info("Output file path list name: " + pathListFileName)
            logger.info("Output file stat file name: " + statListFileName)
            logger.info("Output type to num_size file name: " + type2NumSizeFile)
            logger.info("output type_typeperiod 2 num_size file name: " + typeTypePeriod2NumSizeFile)
            logger.info("Output user to num_size file: " + user2UsageFile)
            logger.info("Output group to to num size file: " + grp2UsageFile)
            logger.info("Output group to to user file: " + grp2UserFile)
            genFilePathList(dirToSearch, pathListFileName, ignoredDirs, logger)
            getStatInfo(pathListFileName,statListFileName, statInvalidListFileName, logger)
            genTypeTypePeriod2NumSize(statListFileName, typeTypePeriod2NumSizeFile, type2NumSizeFile, baseTime) 
            gen_user_grp_usage(statListFileName, user2UsageFile, grp2UsageFile, grp2UserFile)
            exit()
